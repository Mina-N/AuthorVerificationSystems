...
class rf_classifier(classifier):
    ...
    def predict(self, author_id):
        ...
        composed_new = np.reshape(composed, (1, -1)) #contains a single sample
        prob = self.rf.predict_proba(composed_new)[0][1]
        return prob
...
class ubm(classifier):
    ...
    def train(self, authors_id, debug=False):
        ...
        for id_, (author, descriptor) in enumerate(zip(authors_id, samples)):
            ...
            ud_new = np.reshape(ud, (1, -1))
            values.append((agm.score(ud_new)/self.bg_classifier.score(ud_new),target))
    ...
    def predict(self, author_id):
        ...
        agm.weights_, agm.means_, agm.covars_ = \
                self.em(ws, ms, cvs,  [descriptor], self.r)
        ud_new = np.reshape(ud, (1, -1))
        if agm.score(ud_new)/self.bg_classifier.score(ud_new) < self.threshold:
