...
def read_docs_to_data(path, files, file_unknown, language):
    ...
    punct_unknown = np.array(punct_unknown) #ADDED
    punct_unknown = punct_unknown.reshape(1, -1)
    av_punct_known = np.array(av_punct_known) #ADDED
    av_punct_known = av_punct_known.reshape(1, -1)
    line_endings_unknown = np.array(line_endings_unknown) #ADDED
    line_endings_unknown = line_endings_unknown.reshape(1, -1)
    av_line_endings_known = np.array(av_line_endings_known) #ADDED
    av_line_endings_known = av_line_endings_known.reshape(1, -1)
    linelength_unknown = np.array(linelength_unknown) #ADDED
    linelength_unknown = linelength_unknown.reshape(1, -1)
    av_linelength_known = np.array(av_linelength_known) #ADDED
    av_linelength_known = av_linelength_known.reshape(1, -1)
...
def vector_similarity(doc1_tokens, doc2_tokens):
    ...
    v1 = np.array(v1) #ADDED
    v1 = v1.reshape(1, -1)
    v2 = np.array(v2) #ADDED
    v2 = v2.reshape(1, -1)
...
def __available_classifiers():
    ...
    available_clfs["svm_gs2"] = Classifier("svm", "Co-best SVM according to Skll Grid Search",
                                           svm.SVC(probability=True, kernel="sigmoid", C=0.01, coef0=0.01, gamma='auto'), #gamma used to be 0.0
                                           True, True, False)
